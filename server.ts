...
async function getAuthUser(req: any, supabase?: any) {
  ...
  if (req && req.headers) {
    ...
    if (authHeader && typeof authHeader === 'string' && authHeader.toLowerCase().startsWith('bearer ') && supabase) {
      try {
  ...
  const isValidId = (val: any) => typeof val === 'string' && val.trim() !== '' && val.trim() !== 'null' && val.trim() !== 'undefined';
      ...
      if (supabase && effectiveUserId) {
        try {
          const { data: dbProfile } = await supabase
            .from('profiles')
            .select('*')
            .or(buildProfileOrFilter(String(effectiveUserId)))
            .maybeSingle();
          ...
          if (dbProfile) {
            userObj = {
              ...dbProfile,
              ...userObj,
              id: dbProfile.id || effectiveUserId,
  ...
  if (req) {
    const rawParam = (isValidId(req.query?.adminId) && req.query.adminId) ||
                     (isValidId(req.query?.userId) && req.query.userId) ||
                     (isValidId(req.query?.steamId) && req.query.steamId) ||
                     (isValidId(req.query?.discordId) && req.query.discordId) ||
                     (isValidId(req.body?.adminId) && req.body.adminId) ||
                     (isValidId(req.body?.userId) && req.body.userId) ||
                     (isValidId(req.body?.steamId) && req.body.steamId) ||
                     (isValidId(req.body?.steamid) && req.body.steamid) ||
                     (isValidId(req.body?.discordId) && req.body.discordId);
      ...
      if (supabase) {
        try {
          const { data: dbProfile } = await supabase
            .from('profiles')
            .select('*')
            .or(buildProfileOrFilter(cleanId))
            .maybeSingle();
...
const getAndSyncGameData = async (supabase: any, title: string, gameId: string, image: string, steamAppId?: number | string) => {
    const safeGameId = (gameId && String(gameId) !== 'undefined' && String(gameId) !== 'null' && String(gameId).trim() !== '') 
      ? String(gameId).trim() 
      : (steamAppId ? `steam_${steamAppId}` : `custom_${Date.now()}`);
...
async function createServer() {
      ...
      if (currentUser && supabase) {
        ...
        try {
          ...
          const { data: existingProfile, error: profileErr } = await supabase
            .from('profiles')
            .select('*')
            .or(`discord_id.eq.${user.id},discord_id.eq.discord_${user.id},steamid.eq.discord_${user.id}`)
            .maybeSingle();

          let sessionUser: any = null;
          let needsReg = false;

          const hasRealSteam = existingProfile && existingProfile.steamid && !String(existingProfile.steamid).startsWith('discord_');
          ...
          if (existingProfile) {
            ...
            sessionUser = {
              ...
              discord_id: user.id,
              provider: 'discord',
              needs_registration: !hasRealSteam
            };
            needsReg = !hasRealSteam;
          } else {
  ...
  app.post('/api/auth/complete-registration', async (req, res) => {
    ...
    try {
      if (!hasSteam) {
        ...
        if (!existingMember) {
          // Check if there's a temporary discord_ profile to convert
          const { data: tempProfile } = await supabase
            .from('profiles')
            .select('*')
            .or(`discord_id.eq.${discordId},steamid.eq.discord_${discordId}`)
            .maybeSingle();

          if (tempProfile) {
            // Update existing temp profile
            await supabase
              .from('profiles')
              .update({
                steamid: targetSteamId,
                steam_name: steamName,
                steam_avatar: steamAvatar || tempProfile.steam_avatar,
                discord_id: discordId,
                discord_name: discordName,
                discord_avatar: discordAvatar,
        ...
        // Reassign any previous submissions, event teams, or adjustments logged under discord_ ID
        const oldDiscordUserIds = [`discord_${discordId}`, discordId];
        for (const oldId of oldDiscordUserIds) {
          try {
            await supabase.from('submissions').update({ user_id: targetSteamId }).eq('user_id', oldId);
            await supabase.from('submissions').update({ steamid: targetSteamId }).eq('steamid', oldId);
            await supabase.from('user_event_teams').update({ steamid: targetSteamId }).eq('steamid', oldId);
            await supabase.from('team_adjustments').update({ user_id: targetSteamId }).eq('user_id', oldId);
          } catch (e) {
            console.warn('[Sync] Non-critical error migrating data to targetSteamId:', e);
          }
        }

        // Recalculate total points for targetSteamId
        await syncUserPoints(supabase, targetSteamId);

        // Update session
        sessionUser.id = targetSteamId;
        sessionUser.steamid = targetSteamId;
        sessionUser.needs_registration = false;
        delete sessionUser.needs_registration;
  ...
  async function ensureEventScoresSaved(supabase: any, eventId: string, forceResync: boolean = false) {
    if (!supabase || !eventId) return;
    try {
      const { data: event } = await supabase
        .from('events')
      ...
      if (!winnerTeam) winnerTeam = event.winner_team;
  ...
  async function syncUserPoints(supabase: any, steamid: string, eventIdToSync?: string) {
    if (!steamid || steamid === 'system_notification' || String(steamid).startsWith('team_pts_')) return null;

    try {
      console.log(`[Sync] Starting sync for user ${steamid}`);

      const cleanSteamId = String(steamid).trim();
      const cleanDiscordId = cleanSteamId.startsWith('discord_') ? cleanSteamId.replace('discord_', '') : cleanSteamId;

      // Lookup candidate IDs for user
      const { data: profile } = await supabase
        .from('profiles')
        .select('steamid, discord_id, id')
        .or(`steamid.eq.${cleanSteamId},steamid.eq.${cleanDiscordId},discord_id.eq.${cleanSteamId},discord_id.eq.${cleanDiscordId},id.eq.${cleanSteamId},id.eq.${cleanDiscordId}`)
        .maybeSingle();

      const candidateIds = Array.from(new Set([
        cleanSteamId,
        cleanDiscordId,
        `discord_${cleanDiscordId}`,
        profile?.steamid ? String(profile.steamid).trim() : null,
        profile?.discord_id ? String(profile.discord_id).trim() : null,
        profile?.discord_id ? `discord_${String(profile.discord_id).trim().replace('discord_', '')}` : null,
        profile?.id ? String(profile.id).trim() : null
      ].filter(Boolean))) as string[];
      
      let targetEventId = eventIdToSync;
      if (!targetEventId) {
      ...
      const orFilterParts = candidateIds.map(cid => {
        const clean = cid.replace('discord_', '');
        return `steamid.eq.${cid},steamid.eq.${clean},discord_id.eq.${cid},discord_id.eq.${clean},id.eq.${cid}`;
      });
      ...
      // If targetEventId exists, also sync event snapshot if it exists or is saved
      if (targetEventId) {
        try {
          await ensureEventScoresSaved(supabase, targetEventId);
        } catch (e) {
          console.warn('[Sync] ensureEventScoresSaved warning:', e);
        }
      }
  ...
  app.get('/api/leaderboard/users', async (req, res) => {
    ...
    try {
      ...
      // Fetch user_event_teams for active event
      ...
      const transformedUsers = eligibleProfiles.map((u: any) => {
  ...
  app.get('/api/leaderboard/event/:eventId', async (req, res) => {
    ...
    try {
      ...
      const { data: allEventSubs, error: subErr } = await supabase
        .from('submissions')
        .select('*');
      ...
      const uetMap = new Map<string, string>();
      (uets || []).forEach((u: any) => {
        if (u.steamid) uetMap.set(String(u.steamid), u.team);
      });
  ...
  // Admin route to forcefully re-sync/recompute event scores from verified submissions & team assignments
  app.post('/api/admin/resync-event-scores', async (req, res) => {
    const supabase = getSupabase();
    if (!supabase) return res.status(500).json({ error: 'Database unavailable' });
    ...
    try {
      await ensureEventWinnerSaved(supabase, eventId);
      await ensureEventScoresSaved(supabase, eventId, true);
  ...
  app.post('/api/submissions', async (req, res) => {
    ...
    try {
      ...
      const submissionData: any = {
        ...
        steam_appid: (finalSteamAppId && !isNaN(parseInt(String(finalSteamAppId)))) ? parseInt(String(finalSteamAppId)) : null,
        points: serverPoints, 
        notes: notes || '',
        status: 'pending',
  ...
  app.put('/api/submissions/:id', async (req, res) => {
    ...
    const { id } = req.params;
    const steamId = String(currentUser.steamid || currentUser.id || currentUser.steam_id || currentUser.discord_id || '');
    const { 
      achievements, 
      hours, 
      achievementsBefore,
  ...
  // Sync user hours and achievements for a specific game on Steam
  app.get('/api/steam/user-game-stats', async (req, res) => {
    try {
      const rawUserId = req.headers['x-user-id'];
      const rawSteamId = req.headers['x-steam-id'];
      const appIdQuery = req.query.appId || req.query.steamAppId;
      ...
      // Look up user profile if steamId not sent in header
      if (!steamId && userId && supabase) {
        const { data: profile } = await supabase
          .from('profiles')
          .select('steamid, owned_games')
