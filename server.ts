...
async function createServer() {
  ...
  const adminOnly = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (!profile || (profile.role !== 'admin' && profile.role !== 'admins')) {
      console.log(`[Admin Auth] Denied: User ${steamId} (${currentUser.displayName}) has role "${profile?.role || 'none'}"`);
      return res.status(403).json({ error: 'Forbidden' });
    }
    console.log(`[Admin Auth] Granted: User ${steamId} (${currentUser.displayName})`);
    next();
  };
