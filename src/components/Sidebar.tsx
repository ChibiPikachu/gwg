...
export default function Sidebar({ userTeam, isAdmin, activeTab, setActiveTab, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className={cn(
        "fixed inset-y-0 left-0 z-[110] dark:bg-[#0c0c0c] bg-white border-r border-black/5 dark:border-white/5 h-screen flex flex-col overflow-y-auto transition-all duration-300 lg:sticky lg:top-0 lg:z-auto shadow-xl dark:shadow-none",
        isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0",
        isCollapsed ? "w-72 lg:w-20 lg:p-4 p-6" : "w-72 p-6"
      )}>
        
        {/* Desktop Collapse Toggle */}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="hidden lg:flex absolute -right-3.5 top-12 items-center justify-center w-7 h-7 rounded-full bg-white dark:bg-[#151515] border border-black/10 dark:border-white/10 text-slate-500 dark:text-white/50 hover:text-slate-900 dark:hover:text-white z-100 transition-all shadow-sm"
        >
          ...
          <div>
            ...
            <div className="space-y-1">
              {menuItems.map((item) => (
                <button
                  key={item.id}
                  title={item.label} // Adds a native hover tooltip when collapsed
