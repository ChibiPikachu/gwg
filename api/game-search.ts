...
async function fetchSteamAppDetails(appIdStr: string) {
  try {
    ...
    // Fallback schema for delisted/F2P/age-gated games
    ...
    // Generic fallback for valid numeric app id
    ...
    if (queryStr) {
      ...
      if (clientId && token) {
        try {
          ...
          // Fallback query using name pattern matching if search index returns empty
          if (!Array.isArray(data) || data.length === 0) {
          ...
          console.warn('[game-search] IGDB search failed:', igdbErr);
        }
      }
      ...
      // If still empty, provide custom fallback option
